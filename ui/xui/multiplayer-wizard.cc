//
// xemu Multiplayer Wizard
//
// Phase 4 — Enhanced online multiplayer
//
// Copyright (C) 2024 OpenXbox contributors
//
...
// Relay service — hostname that a real deployment would point at a
// STUN/TURN/relay server.
#define OPENXBOX_RELAY_HOST "relay.openxbox.app"
...
MultiplayerWizard::MultiplayerWizard()
    : m_mode(MULTIPLAYER_MODE_OPENXBOX),
...
bool MultiplayerWizard::IsDefaultMac()
{
    ...
    // If there are no NICs yet (early startup), conservatively assume default.
...
std::string MultiplayerWizard::RoomCodeToAddress(const char *code)
{
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "%s:%u", OPENXBOX_RELAY_HOST, (unsigned)port);
...
{
    switch (mode) {
    case MULTIPLAYER_MODE_OPENXBOX:
...
void MultiplayerWizard::DrawModeCombo()
{
    ...
    if (ChevronCombo(
            "Scenario",
            &mode_int,
            "Play with another OpenXbox user\0"
...
// ---------------------------------------------------------------------------
// DrawRelaySection — 4B easy-join room code UI (OpenXbox mode only).
// ---------------------------------------------------------------------------
void MultiplayerWizard::DrawRelaySection(bool appearing)
{
    ...
    ImGui::PushFont(g_font_mgr.m_menu_font_small);
    ImGui::TextWrapped(
        "Enter a room code to connect through the OpenXbox relay server. "
...
void MultiplayerWizard::DrawModeDetails(bool appearing)
{
    ...
    case MULTIPLAYER_MODE_OPENXBOX: {
        ...
        PrepareComboTitleDescription(
            "Remote Address",
            "IP:port of the other OpenXbox instance — e.g. 203.0.113.5:3074",
            size_ratio);
    ...
    case MULTIPLAYER_MODE_LAN:
        SectionTitle("Bridged Adapter — Real Xbox on LAN");
        ImGui::PushFont(g_font_mgr.m_menu_font_small);
        ImGui::TextWrapped(
            ...
            "Network interface).\n\n"
            "Make sure every Xbox and every OpenXbox instance on the session "
    ...
    case MULTIPLAYER_MODE_XLINKKAI:
        ...
        ImGui::TextWrapped(
            ...
            "2. Launch the Kai Engine — it creates a virtual network adapter.\n"
    ...
    case MULTIPLAYER_MODE_INSIGNIA:
        ...
        ImGui::TextWrapped(
            ...
            "3. NAT is pre-selected — no further network changes needed.\n"
...
void MultiplayerWizard::Draw()
{
    ...
    // Relay HUD indicator shown inline whenever a relay session is active.
    if (m_relay_active) {
        Separator();
        ImGui::PushFont(g_font_mgr.m_menu_font_small);
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.2f, 1.0f, 0.4f, 1.0f));
        ImGui::Text(ICON_FA_SERVER "  Relay active — latency: %.0f ms   "
                    "packet loss: %.1f%%",
